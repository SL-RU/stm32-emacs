...
(defcustom stm32-template-dir "cmake"
  "Name of folder with cmake files."
  :group 'stm32
  :type 'string)
...
(defcustom stm32-cmd-cmake
  ...
  "Command to generate project in build directory.  To use Ninja set to 'cmake .. -DCMAKE_BUILD_TYPE=Debug -G Ninja'."
  :group 'stm32
  :type 'string)
  ...
  (let* ((fil (read-directory-name "Select STM32CubeMx directory:"))
	...
	(progn
	  (message (concat "copying " stm32-template))
          ;; Copy directory cmake
          (when (or (not (file-exists-p (concat fil stm32-template-dir)))
                    (y-or-n-p (concat "Directory cmake "
			              " already exists. Overwrite? "
                                      "with template?")))
	    (progn (copy-directory (concat stm32-template stm32-template-dir)
                                   (concat fil stm32-template-dir) t t nil)
                   (message (concat "copied " (concat fil stm32-template-dir)))))
          ;; Copy files CMakeLists.txt and CMakeIgnore.txt
...
(defun stm32-run-openocd-gdb-custom-path ()
  "Start openocd server and select executable manually."
  (interactive)
  (let ((p (get-buffer-process "*openocd*")))
    (when p
      (if (y-or-n-p "Kill currently running openOCD? ")
  ...
  (let* ((elf (read-file-name "Select .elf file"))
	 (c stm32-openocd-command)
	 (ocd (read-file-name "Select openocd.cfg")))
    (with-temp-buffer
      "*openocd*" (async-shell-command (concat c " -f " ocd)
				       "*openocd*"
				       "*Messages*")
      (if (process-live-p (get-buffer-process "*openocd*"))
	  (progn (message "openocd started")
                 (gdb (concat stm32-openocd-gdb-start elf)))
	(message "openocd failed to start"))
      )))
